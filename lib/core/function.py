...
def validate(epoch, config, val_loader, val_dataset, model, criterion, output_dir,
    ...
    for batch_i, (img, target, paths, shapes) in tqdm(enumerate(val_loader), total=len(val_loader)):
        ...
        with torch.no_grad():
            ...
            _, ll_predict = torch.max(ll_seg_out[0], 1)
            ...
            total_loss, head_losses = criterion(ll_seg_out, target, shapes, model)  # Compute loss
            ...
            if config.TEST.PLOTS:
                if batch_i == 0:
                    for i in range(test_batch_size):
                        ...
                        ll_seg_mask = ll_seg_out[0][i][:, pad_h:height - pad_h, pad_w:width - pad_w].unsqueeze(0)
                        ll_seg_mask = torch.nn.functional.interpolate(ll_seg_mask, scale_factor=int(1 / ratio),
                                                                      mode='bilinear')
                        _, ll_seg_mask = torch.max(ll_seg_mask, 1)

                        ll_gt_mask = target[0][i][:, pad_h:height - pad_h, pad_w:width - pad_w].unsqueeze(0)
    ...
    return ll_segment_result, losses.avg, maps, t
